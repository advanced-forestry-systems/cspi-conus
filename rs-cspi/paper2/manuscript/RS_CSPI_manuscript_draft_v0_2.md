...
Per target a random forest (Breiman 2001), fit with ranger (Wright and Ziegler 2017) in R 4.4.0, is fit from the stack and predicted across all forested cells: 500 trees for the per-target fits and 300 for the grid and cross-validation models, mtry the default floor(p/3), and a fixed random seed for reproducibility. Out-of-bag R-squared and root mean square error are reported. To test the inventory plot frame, each target is fit twice, once at the 61,656 plot coordinates and once wall to wall on the predictor grid.
...
During the preparation of this work the author used a large language model based assistant for code organization, consistency checking, and copy-editing. The author reviewed and edited the content as needed and takes full responsibility for the content of the publication.
...
Figure 3. The height-versus-flux divergence in the inventory plots themselves: FIA site index versus satellite net primary productivity (a, Pearson -0.38) and flux-derived gross primary productivity (b, Pearson -0.42), across roughly 61,655 plots.
